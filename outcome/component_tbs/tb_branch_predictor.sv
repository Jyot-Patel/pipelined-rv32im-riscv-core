...
// ============================================================================
// tb_branch_predictor — SoC-level exclusive branch prediction accuracy tb
// Active core: rtl/core/rv32i_core.sv + rtl/units/branch_predictor.sv
// Legacy:     rtl/rv32i_core.sv (ignored)
// Purpose: measures prediction accuracy per-branch by sampling EX stage
//          compare predicted (idex_pred_*) vs actual (ex_branch_taken/target)
//          cross-checks DUT ex_bp_mispredict.
// See documents/branch_predictor_control.md Ch.5 for verification method.
// ============================================================================
module tb_branch_predictor;
  ...
  // --------------------------------------------------------------------------
  // Use hierarchical refs into active core rtl/core/rv32i_core.sv
  ...
  initial begin
    ...
    run_to_halt(4000);
    // if infinite loop still running after halt timeout, just sample current counters
    check_results();
    #10;
    $finish;
  end
